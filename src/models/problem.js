...
function validateProblemItems(problem) {
...
const problemSchema = new mongoose.Schema({
  ...
  editorial: {
    // should be required to publish problem? not sure
    type: String,
  },
  testSets: {
    type: [testSetSchema],
    ...
    validate: {
      ...
      message:
        "testSets should be an array of testsets with at least one valid testSet",
    },
  },
  isPublished: {
    type: Boolean,
    ...
    validate: {
      validator: function (value) {
        ...
        if (
          ...
          !this.validator ||
          !this.testSets ||
          !Array.isArray(this.testSets) ||
          this.testSets.length == 0
        )
          return false;
        return true;
      },
  ...
  modelAnswer: {
    // should be required before publishing problem
    type: ObjectId,
    ref: "Code",
    required: function () {
      // if is published then code should be required
...
async function updateProblem(id, updated) {
  // this object should contain the items to update
  try {
